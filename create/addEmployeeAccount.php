...
if($conn->query($sql_query) !== TRUE) {
    ...
    //create an array with 'error' tag that maps to the error,
    //this is so the json can encode as [{"error" : "..."}]
    $arr = array('status' => 'error', 'message' => 'Could not insert employee into the database!', 'stackTrace' => $conn->error);
    echo json_encode($arr);
    die();
}
...
if($results->num_rows > 1 || $results->num_rows == 0) {
    $arr = array('status' => 'error', 'message' => 'Could not get new employeeID!', 'stackTrace' => $conn->error);
    echo json_encode($arr);
    die();
}
